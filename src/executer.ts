...
Modified: 2022-01-31T07:36:02.273Z
*/
...
import { Argument, Command, Insert, mArguments, Option } from './core'
	...
	return await import(path).then(m => {
		...
		const nclass = inFly[fname]
			...
			Insert(
				mArguments,
				new Argument(
				{kind: 'boolean', defaults: undefined, optional: false, property: 'help', rest: false},
				'',
				undefined,
				0
				),
...
export async function ExecuterDirector(subcommands: boolean) {
	...
	if(help.match(programArgs) || !commandCtr) {
		...
		const mappedOptions = options.map( opt => ({ index: opt.index, tag: opt.name, value: opt.match(programArgs)}))
		
		const mappedArguments = argus.map( opt => ({ index: opt.index, tag: opt.name, value: opt.match(programArgs)}))
		const final_args = [...mappedArguments, ...mappedOptions].sort( (a, b) => a.index - b.index ).map(arg => arg.value)
